...
struct FeatureInfo {
  ...
  FeatureInfo() = default;
  FeatureInfo(
      const char* tempCompatibility,
      const char* tempFeatureVersion,
      const char* tempAffectedComponent,
      const char* tempAffectedComponentVersion,
      const char* tempInfoLog) {
    ...
    std::all_of(
        copyList.begin(),
        copyList.end(),
        [](std::tuple<const char*, char*>& copyNode) {
