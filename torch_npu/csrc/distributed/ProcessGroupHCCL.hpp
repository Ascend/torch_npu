...
//
// Example on using the HCCL process group, use ProcessGroupHCCL pg(store, rank,
// size) to create, and use std::shared_ptr<WorkHCCL> work =
// pg.allreduce(tensors) to start a work.
//
...
class ProcessGroupHCCL : public c10d::Backend {
 public:
  ...
  std::string getHcclCommNameWithoutInit(
      int rankid,
 ...
 protected:
  ...
  // The store keys to trace the last HCCL collective kernel Ascend events -
  ...
  struct CommStruct {
    ...
    bool operator<(const CommStruct& other) const {
      return std::tie(comm_name, op_name) <
          std::tie(other.comm_name, other.op_name);
    }
  };
