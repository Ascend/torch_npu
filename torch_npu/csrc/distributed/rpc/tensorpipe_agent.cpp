...
std::vector<c10::Stream> getStreamsFromPoolForDevices(
  ...
  for (const c10::Device& device : devices) {
    TORCH_INTERNAL_ASSERT(
        device.type() == impl.type(), DIST_ERROR(ErrCode::PARAM));
...
TensorPipeAgent::TensorPipeAgent(
    ...
    : RpcAgent(
      ...
      context_(std::make_shared<tensorpipe_npu::Context>(
          tensorpipe_npu::ContextOptions().name(workerInfo_.name_))),
      rankToNameStore_("names", store),
      nameToAddressStore_("addrs", store),
      shutdownStore_("shutdown", store) {
  tensorpipe_npu::setDeviceId(
...
void TensorPipeAgent::onListenerAccepted(
    ...
    std::shared_ptr<tensorpipe_npu::Pipe>& pipe) {
  if (error) {
    if (error.isOfType<tensorpipe_npu::ListenerClosedError>() &&
...
void TensorPipeAgent::pipeWrite(
  ...
  pipe->write(
      ...
      [tpBuffers{
       ...
       fn{std::move(fn)},
       streams{std::move(streams)}](const tensorpipe_npu::Error& error) {
        fn(error);
      });
}
        ...
        threadPool_.run([this,
          ...
          futureResponseMessage->addCallback(
              [this, pipe, messageId](
                ...
                VLOG(1)
                    << "FutureResponseMessage set deciveID="
                    << c10_npu::NpuSysCtrl::GetInstance().InitializedDeviceID()
                    << "pid=" << getpid()
                    << " thread_id=" << std::this_thread::get_id();
                c10_npu::SetDevice(
...
c10::intrusive_ptr<JitFuture> TensorPipeAgent::send(
    ...
    if (it == connectedPipes_.end()) {
      ...
      std::tie(it, std::ignore) = connectedPipes_.emplace(
          ...
          std::forward_as_tuple(context_->connect(
              url,
              tensorpipe_npu::PipeOptions().remoteName(toWorkerInfo.name_))));
    }
  }
  ...
  pipeWrite(
      ...
      [this, &clientPipe, messageId](
          const tensorpipe_npu::Error& error) mutable {
        if (error) {
          if (error.isOfType<tensorpipe_npu::PipeClosedError>() &&
              !rpcAgentRunning_.load()) {
            // This is expected.
          } else {
            LOG(WARNING) << "RPC agent for " << workerInfo_.name_
