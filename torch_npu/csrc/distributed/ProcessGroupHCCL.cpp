...
void syncStreams(
  ...
  for (size_t i = 0; i < devices.size(); ++i) {
    ...
    ASCEND_LOGI(
        "Event: record hccl group is successfully executed, event=%p",
        hcclEvent.event());
    hcclEvent.block(hcclStream);
    ASCEND_LOGI(
        "Event: block hccl group is successfully executed, event=%p",
        hcclEvent.event());
  }
}
...
void exceptionCallback(aclrtExceptionInfo* exceptionInfo) {
  // notice: Do not raise error, otherwise we will get call stacks of the rts
  // callback function.
...
std::ostream& operator<<(
    std::ostream& output,
    const ProcessGroupHCCL::WorkHCCL& workHCCL) {
...
ProcessGroupHCCL::WorkHCCL::WorkHCCL(
  ...
  if (desyncDebug) {
    hcclStartEvents_ =
        std::make_shared<std::vector<c10_npu::NPUEvent>>(devices.size());
  }
...
void ProcessGroupHCCL::WorkHCCL::setException(
...
bool ProcessGroupHCCL::WorkHCCL::checkTimeout(
...
std::chrono::milliseconds GetDispatchTimeout() {
  ...
  uint32_t hccl_exec_timeout =
      c10_npu::option::OptionsManager::GetHCCLExecTimeout();
  if (hccl_exec_timeout > 0) {
    if (hccl_exec_timeout < dispatchTimeout_ + dispatchoffset &&
        hccl_exec_timeout > mindispatchTimeout_ + dispatchoffset) {
...
void ProcessGroupHCCL::WorkHCCL::checkDispatch() {
  if (!*is_dispatched && !is_reported) {
    ...
    if (timeElapsed > dispatchTimeout_) {
      ...
      ASCEND_LOGE(
          "Process group work %s, seq_num %u dispatch timeout. %s",
          opTypeToString(opType_).c_str(),
          seq_,
          repo_info.c_str());
      is_reported = true;
    }
    ...
    ASCEND_LOGE(
        "Process group work %s, seq_num %u dispatch sucess. This error log can be ignored.",
        opTypeToString(opType_).c_str(),
        seq_);
    is_reported = false;
  }
...
void ProcessGroupHCCL::WorkHCCL::handleException(
...
void ProcessGroupHCCL::WorkHCCL::synchronizeInternal(
  ...
  for (const auto i : c10::irange(devices_.size())) {
    ...
    ASCEND_LOGI(
        "Event: block hccl work is successfully executed, event=%p",
        (*hcclEndEvents_)[i].event());
  ...
  if (!recorded_inputs_.empty()) {
    for (auto it = recorded_inputs_.begin(); it != recorded_inputs_.end();
         ++it) {
      auto storage = it->first.lock();
      if (storage) {
        c10_npu::NPUCachingAllocator::eraseStream(
            storage->data_ptr(), it->second);
      }
    }
  }
  if (!recorded_outputs_.empty()) {
    for (auto it = recorded_outputs_.begin(); it != recorded_outputs_.end();
         ++it) {
      auto storage = it->first.lock();
      if (storage) {
        c10_npu::NPUCachingAllocator::eraseStream(
  ...
  if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
    ...
    lazy_destory_tensors_.clear();
      ...
      c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
...
ProcessGroupHCCL::ProcessGroupHCCL(
  ...
  uint32_t hccl_event_timeout =
      c10_npu::option::OptionsManager::GetHCCLEventTimeout();
  if (hccl_event_timeout > 0) {
    ...
    ASCEND_LOGI(
        "Get env HCCL_EXEC_TIMEOUT value %u, and set op wait timeout to %u.",
        hccl_exec_timeout,
        kOpWaitTimeout);
  }
  NPU_CHECK_SUPPORTED_OR_ERROR(
      c10_npu::acl::AclrtSetOpWaitTimeout(kOpWaitTimeout));
    ...
    if (blockingWait != nullptr) {
      ...
      if (val == 1) {
        ...
        throw std::runtime_error(
            ...
            std::string(HCCL_BLOCKING_WAIT) + DIST_ERROR(ErrCode::VALUE));
      }
    }
  ...
  asyncErrorHandling_ = static_cast<ErrorHandlingMode>(
      c10_npu::option::OptionsManager::CheckUseHcclAsyncErrorHandleEnable());
  desyncDebug_ = static_cast<bool>(
      c10_npu::option::OptionsManager::CheckUseDesyncDebugEnable());
...
void ProcessGroupHCCL::workCleanupLoop() {
  ...
  while (!done || !terminateProcessGroup_.load()) {
    ...
    for (auto it = workMetaList_.begin(); it != workMetaList_.end();
      ...
      if (needSetDevice) {
        NPU_CHECK_ERROR(
            c10_npu::SetDevice(static_cast<int>(work.devices_[0].index())));
        needSetDevice = false;
      }
      ...
      if (work.exception()) {
        ...
        if (desyncDebug_ && timedOut) {
          try {
            ...
            LOG(ERROR)
                << "Failed to rerieve HCCL_DESYNC_DEBUG report with unknown error."
      ...
      if (work.isCompleted()) {
        if (*(work.is_dispatched) && work.is_reported) {
          ASCEND_LOGE(
              "Process group work %s, seq_num %u dispatch sucess. This error log can be ignored.",
              opTypeToString(work.opType_).c_str(),
              work.seq_);
...
void ProcessGroupHCCL::broadcastMasterID(HcclRootInfo* hcclID) {
  ...
  if (rank_ == 0) {
    ...
    try {
      ...
      TORCH_CHECK(
          vec.size() == HCCL_ROOT_INFO_BYTES, DIST_ERROR(ErrCode::PARAM));
      std::memcpy(hcclID, vec.data(), vec.size());
    } catch (const std::exception& e) {
      throw std::runtime_error(
          "store->get() got error: " + std::string(HCCL_BLOCKING_WAIT) +
          DIST_ERROR(ErrCode::INTERNAL));
    } catch (...) {
      throw std::runtime_error(
          "Unknown exception: " + std::string(HCCL_BLOCKING_WAIT) +
          DIST_ERROR(ErrCode::INTERNAL));
    }
...
void ProcessGroupHCCL::recordDataVol(
    std::string opName,
    const std::string dataVol,
    const int currRank,
  ...
  TORCH_CHECK(
      master_addr != nullptr,
      "Unable to fetch master IP addr, environment variable is null.",
      DIST_ERROR(ErrCode::NOT_FOUND));
  fileName << master_addr << "_" << commName << "_" << std::to_string(currRank)
           << ".log";
  std::string out_file_path = c10::str(nslb_path, "/", fileName.str());
  try {
    if (access(nslb_path, W_OK) != 0 &&
        mkdir(nslb_path, S_IRWXU | S_IRGRP | S_IXGRP) != 0) {
      throw std::exception();
    }
    outfile.open(out_file_path, std::ios::app);
  } catch (std::exception& e) {
    throw std::runtime_error(
        "Open shared directory failed. Please check whether input path is valid." +
        DIST_ERROR(ErrCode::NOT_FOUND));
  }
...
void ProcessGroupHCCL::recordComm(
    std::string filename,
    std::string opName,
    const int currRank,
    std::vector<std::shared_ptr<HCCLComm>>& hcclComms) {
  ...
  try {
    ...
    throw std::runtime_error(
        "Open shared directory failed. Please check whether filename is valid." +
        DIST_ERROR(ErrCode::NOT_FOUND));
  }
  std::transform(opName.begin(), opName.end(), opName.begin(), ::tolower);
...
std::vector<std::shared_ptr<HCCLComm>>& ProcessGroupHCCL::getHCCLComm(
  ...
  for (size_t i = 0; i < devices.size(); ++i) {
    ...
    int rank =
        getRank() * static_cast<int>(devices.size()) + static_cast<int>(i);
...
int64_t ProcessGroupHCCL::getStreamId(bool p2p) {
  ...
  std::vector<at::Device> devices = {
      at::Device(c10::DeviceType::PrivateUse1, device)};
  auto key = getKeyFromDevices(devices);
  if (p2p && hcclCommInitRootInfoConfigExist() &&
      (c10_npu::option::OptionsManager::GetP2PBufferSize() != 0)) {
...
void check_npu_tensors_different_devices(
    const std::vector<at::Tensor>& tensors) {
  if (tensors.size() == 0) {
    TORCH_CHECK(
        false, "Tensor list must be nonempty", DIST_ERROR(ErrCode::PARAM));
  }
  ...
  for (const auto& t : tensors) {
    if (!torch_npu::utils::is_npu(t) || t.is_sparse()) {
      TORCH_CHECK(
          false, "Tensors must be NPU and dense", DIST_ERROR(ErrCode::TYPE));
    }
    if (t.scalar_type() != first.scalar_type()) {
      TORCH_CHECK(
          false, "Tensors must have identical type", DIST_ERROR(ErrCode::TYPE));
    }
    if (t.sizes() != first.sizes()) {
      TORCH_CHECK(
          false, "Tensors must have identical size", DIST_ERROR(ErrCode::TYPE));
    }
    if (t.strides() != first.strides()) {
      TORCH_CHECK(
          false,
          "Tensors must have identical strides",
          DIST_ERROR(ErrCode::TYPE));
    }
    if (!t.is_contiguous(t.suggest_memory_format())) {
      TORCH_CHECK(
          false, "Tensors must be contiguous", DIST_ERROR(ErrCode::TYPE));
    }
    const auto inserted = usedDevices.insert(t.get_device()).second;
    if (!inserted) {
      TORCH_CHECK(
          false,
          "Tensors must be on distinct NPU devices",
          DIST_ERROR(ErrCode::TYPE));
    }
  }
}
...
std::vector<at::Tensor> cast_to_origin_format(
  ...
  for (auto& tensor : inputTensors) {
    if (at_npu::native::FormatHelper::IsBaseFormatType(tensor)) {
      ...
      auto origin_format = torch_npu::NPUBridge::GetNpuStorageImpl(tensor)
                               ->npu_desc_.origin_format_;
      inputTensors_[index] =
          at_npu::native::custom_ops::npu_format_cast(tensor, origin_format);
    }
    index++;
  }
...
std::vector<at::Tensor> create_base_format_tensors(
    const std::vector<at::Tensor>& inputTensors) {
  std::vector<at::Tensor> inputTensors_;
  inputTensors_.resize(inputTensors.size());
...
void nslb_record_end() {
  ...
  end_file_path = c10::str(
      nslb_path, "/end_", getenv("MASTER_ADDR"), "_", getpid(), ".log");
  try {
    if (access(nslb_path, W_OK) != 0 &&
        mkdir(nslb_path, S_IRWXU | S_IRGRP | S_IXGRP) != 0) {
      throw std::exception();
    ...
    throw std::runtime_error(
        "NSLB set end failed." + DIST_ERROR(ErrCode::NOT_FOUND));
  }
}
...
c10::intrusive_ptr<ProcessGroupHCCL::WorkHCCL> ProcessGroupHCCL::initWork(
  ...
  return c10::make_intrusive<ProcessGroupHCCL::WorkHCCL>(
      devices, rank, opType, seq_, desyncDebug_);
}

void ProcessGroupHCCL::workEnqueue(
...
int64_t ProcessGroupHCCL::getHcclComm(int rankid) {
  ...
  TORCH_CHECK(
      hcclComms.size() == 1,
      "expect hcclComms.size() = 1, but hcclComms.size() = ",
      hcclComms.size(),
      DIST_ERROR(ErrCode::VALUE));
  auto ret_hcom = hcclComms[0]->getHcclComm();
  int64_t hccl_comm =
      static_cast<int64_t>(reinterpret_cast<intptr_t>(ret_hcom));
  return hccl_comm;
}
...
std::string ProcessGroupHCCL::getHcclCommName(int rankid) {
  ...
  TORCH_CHECK(
      ...
      numNPUs,
      DIST_ERROR(ErrCode::VALUE));
  c10::DeviceIndex indexFromRank =
      static_cast<c10::DeviceIndex>(rankid % numNPUs);
  c10::DeviceIndex indexFromCurDevice = c10_npu::current_device();
  if (indexFromRank != indexFromCurDevice) {
    std::string warning_message = "The indexFromRank " +
        std::to_string(indexFromRank) + "is not equal indexFromCurDevice " +
        std::to_string(indexFromCurDevice) +
  ...
  at::Device device =
      at::Device(c10::DeviceType::PrivateUse1, indexFromCurDevice);
  std::vector<at::Device> devices = {device};
  const auto key = getKeyFromDevices(devices);
  auto& hcclComms = getHCCLComm(key, devices);
  TORCH_CHECK(
...
std::string ProcessGroupHCCL::getHcclCommNameWithoutInit(
    int rankid,
    std::vector<std::shared_ptr<HCCLComm>>& hcclComms) {
  TORCH_CHECK(
      hcclComms.size() == 1,
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::collective(
  ...
  auto key = getKeyFromDevices(devices);
  auto hcclComms = getHCCLComm(key, devices);
  if (hcclCommInitRootInfoConfigExist() &&
      c10_npu::option::OptionsManager::GetP2PBufferSize() != 0) {
    ...
    if (opType == c10d::OpType::SEND || opType == c10d::OpType::RECV ||
  ...
  if (nslb_path != nullptr && !nslb_is_end) {
    ...
    if (seq_ <= nslb_num) {
      ...
      TORCH_CHECK(
          global_rank != nullptr,
          "Unable to fetch global rank for NSLB.",
          DIST_ERROR(ErrCode::NOT_FOUND));
      recordDataVol(
          opTypeToString(opType),
          std::to_string(dataVol),
          atoi(global_rank),
          hcclComms);
    }
    if (seq_ >= nslb_num) {
  ...
  static bool perf_dump_enable =
      c10_npu::option::OptionsManager::CheckPerfDumpEnable();
  if (perf_dump_enable) {
    if (filepath.empty()) {
      auto pid = getpid();
      ...
      if (realpath(perfDumpPath.c_str(), abs_path) == nullptr) {
        TORCH_CHECK(
            0, "perfDumpPath is not realpath.", DIST_ERROR(ErrCode::NOT_FOUND));
      }
      ...
      if (isFileExists(path_temp)) {
        ...
        try {
          ...
          throw std::runtime_error(
              "Open shared directory failed. Please check whether filepath is valid." +
              DIST_ERROR(ErrCode::NOT_FOUND));
        }
    ...
    if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
      ...
      if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
        ...
        work->recorded_inputs_.push_back(std::make_pair(
            inputs[i].storage().getWeakStorageImpl(), hcclStream));
      }
    }
  }
  {
    for (const auto i : c10::irange(inputs.size())) {
      ...
      if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
        ...
        work->recorded_outputs_.push_back(std::make_pair(
            outputs[i].storage().getWeakStorageImpl(), hcclStream));
      } else if (
          c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
          c10_npu::option::AVOID_RECORD_STREAM) {
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    ...
    ASCEND_LOGI(
        "Event: record hccl work is successfully executed, event=%p",
        (*(work->hcclEndEvents_))[i].event());
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::collective(
  ...
  return collective(
      ...
      [](std::vector<c10_npu::NPUStream>&,
         c10::intrusive_ptr<ProcessGroupHCCL::WorkHCCL>&) {},
      [](std::vector<c10_npu::NPUStream>&,
         c10::intrusive_ptr<ProcessGroupHCCL::WorkHCCL>&) {},
      opType);
}
  ...
  return collective(
      ...
      [&](std::vector<c10_npu::NPUStream>& hcclStreams,
        ...
        if (tensors[0].scalar_type() == at::kBool ||
          ...
          tensors_cp[0] =
              at_npu::native::custom_ops::npu_dtype_cast(tensors[0], at::kInt);
        }
      },
      [&](std::vector<c10_npu::NPUStream>& hcclStreams,
          c10::intrusive_ptr<ProcessGroupHCCL::WorkHCCL>&) {
        if (tensors_cp[0].scalar_type() != tensors[0].scalar_type()) {
          c10_npu::NPUStreamGuard guard(hcclStreams[0]);
          c10_npu::NPUCachingAllocator::recordStream(
              tensors_cp[0].storage().data_ptr(), hcclStreams[0]);
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::batch_isend_irecv(
  ...
  return collective(
      ...
      [&](at::Tensor& input,
        ...
        auto hccl_call = [tensor_ptr_list,
                          numel_list,
                          type_list,
                          remote_rank_list,
                          op_type,
          ...
          for (size_t i = 0; i < op_type.size(); ++i) {
            ...
            sendRecvInfo[i] = HcclSendRecvItem{
                currType,
                tensor_ptr_list[i],
                numel_list[i],
                type_list[i],
          ...
          auto hccl_result = hcclBatchIsendIrecv(
              sendRecvInfo, itemNum, comm, stream.stream(false));
          *is_dispatched = true;
          return hccl_result;
        };
        at_npu::native::OpCommand cmd;
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::broadcast(
  ...
  return collective(
      ...
      [&](at::Tensor& input,
        ...
        auto hcclType = getHcclDataType(input.scalar_type());
        auto hccl_call =
            [inputDataPtr, numel, hcclType, root, comm, stream, is_dispatched]()
            -> int {
          auto hccl_result = HcclBroadcast(
              inputDataPtr, numel, hcclType, root, comm, stream.stream(false));
          *is_dispatched = true;
          return hccl_result;
        };
        at_npu::native::OpCommand cmd;
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::_reduce_oop(
  ...
  if (outputTensor.numel() != inputTensor.numel()) {
    TORCH_CHECK(
        false,
        "output tensor must have the same numel as input tensor",
        DIST_ERROR(ErrCode::PARAM));
  }
  uint64_t rank = opts.rootRank;
  ...
  return collective(
      ...
      [&](std::vector<c10_npu::NPUStream>& hcclStreams,
          c10::intrusive_ptr<ProcessGroupHCCL::WorkHCCL>&) {
        if (inputTensors[0].scalar_type() == at::kBool ||
            inputTensors[0].scalar_type() == at::kByte) {
          c10_npu::NPUStreamGuard guard(hcclStreams[0]);
          inputTensors[0] = at_npu::native::custom_ops::npu_dtype_cast(
              inputTensors[0], at::kInt);
        }
        if (outputTensors[0].scalar_type() == at::kBool ||
            outputTensors[0].scalar_type() == at::kByte) {
          c10_npu::NPUStreamGuard guard(hcclStreams[0]);
          outputTensors[0] = at_npu::native::custom_ops::npu_dtype_cast(
              outputTensors[0], at::kInt);
        }
      },
      [&](std::vector<c10_npu::NPUStream>& hcclStreams,
          c10::intrusive_ptr<ProcessGroupHCCL::WorkHCCL>&) {
        if (outputTensors[0].scalar_type() != outputTensor.scalar_type()) {
          c10_npu::NPUStreamGuard guard(hcclStreams[0]);
          c10_npu::NPUCachingAllocator::recordStream(
              outputTensors[0].storage().data_ptr(), hcclStreams[0]);
...
at::Tensor ProcessGroupHCCL::byte_alignment(at::Tensor& tensors) {
  ...
  if (num_add != 0) {
    ...
    if (inter_tensors.scalar_type() == at::ScalarType::Bool) {
      inter_tensors = at_npu::native::custom_ops::npu_dtype_cast(
          inter_tensors, at::ScalarType::Int);
      transflag = true;
    }
    ...
    if (transflag == true) {
      inter_tensors = at_npu::native::custom_ops::npu_dtype_cast(
          inter_tensors, at::ScalarType::Bool);
    }
  }
  return inter_tensors;
}
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::allgather(
  ...
  if (same_size) {
    ...
    std::vector<at::Tensor> byte_alignment_inputTensors_ = {
        byte_alignment(inputTensors_[0])};
    std::vector<at::Tensor> byte_alignment_outputTensors_;
    for (unsigned int i = 0; i < outputTensors[0].size(); i++) {
      byte_alignment_outputTensors_.push_back(
          byte_alignment(outputTensors[0][i]));
    }
    ...
    auto outputFlattened = flatten_for_scatter_gather(
        byte_alignment_outputTensors, byte_alignment_inputTensors_, size_);
    ...
    return collective(
        ...
        [&](at::Tensor& input,
          ...
          auto hccl_call = [inputDataPtr,
                            ...
                            stream,
                            is_dispatched]() -> int {
            auto hccl_result = HcclAllGather(
                inputDataPtr,
                outputDataPtr,
                numel,
                hcclType,
        ...
        [&](std::vector<c10_npu::NPUStream>& hcclStreams,
          ...
          for (const auto i : c10::irange(outputTensors.size())) {
            ...
            for (const auto j : c10::irange(outputTensors[0].size())) {
              // See [Sync Streams].
              if (c10_npu::option::OptionsManager::
                      GetMultiStreamMemoryReuse() ==
                  c10_npu::option::AVOID_RECORD_STREAM) {
                work->stashed_for_allocator_safety_.push_back(
                    outputTensors[i][j]);
              } else {
                c10_npu::NPUCachingAllocator::recordStream(
              ...
              at::Tensor output_tensor =
                  outputFlattened[i][j].slice(1, 0, output_nums[j]);
              at::Tensor output_tensor_shape =
                  at::reshape(output_tensor, outputTensors[i][j].sizes());
    ...
    TORCH_NPU_WARN_ONCE(
        "The current allgather operator has a defect in handling different tensor shape, \
    ...
    for (const auto i : c10::irange(num_reduces)) {
      ...
      auto work = collective(
          ...
          [&](at::Tensor& input,
              ...
              std::shared_ptr<bool> is_dispatched) {
            RECORD_FUNCTION("HcclBroadcast", std::vector<c10::IValue>({input}));
            const auto root = broadcastOpts.rootRank * inputs_multi_dev.size() +
                broadcastOpts.rootTensor;

            auto inputDataPtr = input.data_ptr();
            auto numel = getNumelForHCCL(input);
            auto hcclType = getHcclDataType(input.scalar_type());
            auto hccl_result = HcclBroadcast(
                inputDataPtr, numel, hcclType, root, comm, stream.stream());
            *is_dispatched = true;
            return hccl_result;
          },
          c10d::OpType::BROADCAST);
    ...
    auto fake_work =
        initWork(getDeviceList(inputTensors), rank_, c10d::OpType::ALLGATHER);
    return fake_work;
  }
}
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::allgather_togather(
  ...
  return collective(
      ...
      [&](at::Tensor& input,
          ...
          std::shared_ptr<bool> is_dispatched) {
        RECORD_FUNCTION(
            "HcclAllgatherTogather", std::vector<c10::IValue>({input}));
        if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() !=
            c10_npu::option::AVOID_RECORD_STREAM) {
          c10_npu::NPUCachingAllocator::recordStream(
              output.storage().data_ptr(), stream);
  ...
  if (same_size) {
    ...
    return collective(
        ...
        [&](at::Tensor& input,
          ...
          checkSupportedDataType(hcclType, functionName);
          RECORD_FUNCTION(
              "HcclReduceScatter", std::vector<c10::IValue>({input}));
          if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() !=
              c10_npu::option::AVOID_RECORD_STREAM) {
            c10_npu::NPUCachingAllocator::recordStream(
                output.storage().data_ptr(), stream);
    ...
    TORCH_NPU_WARN_ONCE(
        "The current reduce_scatter operator has a defect in handling different tensor shape,",
    ...
    // Create a fake_work for python side;
    auto fake_work = initWork(
        getDeviceList(outputTensors), rank_, c10d::OpType::REDUCE_SCATTER);
    return fake_work;
  }
}
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::_reduce_scatter_base(
  ...
  return collective(
      ...
      [&](at::Tensor& input,
        ...
        checkSupportedDataType(hcclType, functionName);
        RECORD_FUNCTION(
            "HcclReduceScatterBase", std::vector<c10::IValue>({input}));
        auto inputDataPtr = input.data_ptr();
        auto outputDataPtr = output.data_ptr();
        auto numel = getNumelForHCCL(output);
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::barrier(
  ...
  if (usedDeviceIdxs_.empty()) {
    auto numNPUs = c10_npu::device_count();
    int16_t deviceIdx =
        static_cast<int16_t>(rank_ % std::max(static_cast<int>(numNPUs), 1));
    devices.push_back(at::Device(c10::DeviceType::PrivateUse1));
  } else {
    for (auto usedDeviceIdx : usedDeviceIdxs_) {
      devices.push_back(
          at::Device(c10::DeviceType::PrivateUse1, usedDeviceIdx));
    }
  }
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::gather(
  ...
  throw std::runtime_error(
      "ProcessGroupHCCL does not support gather" +
      DIST_ERROR(ErrCode::NOT_SUPPORT));
}
      ...
      [&](std::vector<c10_npu::NPUStream>& hcclStreams,
        ...
        for (size_t i = 0; i < tensors_.size(); ++i) {
          ...
          if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
            ...
            c10_npu::NPUCachingAllocator::recordStream(
                tensors_[i].storage().data_ptr(), hcclStreams[i]);
            if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
                c10_npu::option::ERASE_RECORD_STREAM) {
              work->recorded_outputs_.push_back(std::make_pair(
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::recvAnysource(
  ...
  TORCH_CHECK(
      false,
      "ProcessGroupHCCL does not support recv",
      DIST_ERROR(ErrCode::NOT_SUPPORT));
}
...
void check_split_sizes(
  ...
  if (split_sizes.empty()) {
    ...
    TORCH_CHECK(
        sum == tensor.size(0),
        "Split sizes dosen't match total dim 0 size",
        DIST_ERROR(ErrCode::TYPE));
  }
}
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::alltoall_base(
  ...
  TORCH_CHECK(
      ranks > 0,
      "Invalid rank count within current process group",
      ranks,
      DIST_ERROR(ErrCode::PARAM));
  std::vector<at::Tensor> inputTensors = {inputTensor};
  std::vector<at::Tensor> outputTensors = {outputTensor};
  ...
  if (inputSplitSizes.empty() && outputSplitSizes.empty()) {
    ...
    TORCH_CHECK(
        ...
        "Tensor's dim 0 does not divide equally across group size",
        DIST_ERROR(ErrCode::PARAM));
    uint64_t output_counts =
        static_cast<uint64_t>(outputTensor.numel() / ranks);
        ...
        [&](std::vector<c10_npu::NPUStream>& hcclStreams,
          ...
          for (size_t i = 0; i < outputTensors_.size(); ++i) {
            ...
            if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
              ...
              c10_npu::NPUCachingAllocator::recordStream(
                  outputTensors_[i].storage().data_ptr(), hcclStreams[i]);
              if (c10_npu::option::OptionsManager::
                      GetMultiStreamMemoryReuse() ==
                  c10_npu::option::ERASE_RECORD_STREAM) {
                work->recorded_outputs_.push_back(std::make_pair(
                    outputTensors_[i].storage().getWeakStorageImpl(),
                    hcclStreams[i]));
              }
            }
            if (!at_npu::native::FormatHelper::IsBaseFormatType(
                    outputTensors[i])) {
    ...
    int inputRowSize = static_cast<int>(
        inputTensor.size(0) ? inputTensor.numel() / inputTensor.size(0) : 1);
    int outputRowSize = static_cast<int>(
        outputTensor.size(0) ? outputTensor.numel() / outputTensor.size(0) : 1);
    ...
    for (int i = 0; i < outSize; i++) {
      outputCounts.push_back(
          static_cast<uint64_t>(outputSplitSizes[i] * outputRowSize));
      if (i > 0) {
        outputSpl.push_back(outputSpl[i - 1] + outputCounts[i - 1]);
      }
    }
    for (int i = 0; i < inputSize; i++) {
      inputCounts.push_back(
          static_cast<uint64_t>(inputSplitSizes[i] * inputRowSize));
      if (i > 0) {
...
c10::intrusive_ptr<c10d::Work> ProcessGroupHCCL::alltoall(
  ...
  for (size_t i = 0; i < input_tensors.size(); i++) {
    ...
    input_tensors_flattened.push_back(
        at::reshape(input_tensors[i], {input_tensors[i].numel(), 1}));
  }
  for (size_t i = 0; i < output_tensors.size(); i++) {
    output_split_sizes.push_back(output_tensors[i].numel());
    output_tensors_flattened.push_back(
        at::reshape(output_tensors[i], {output_tensors[i].numel(), 1}));
  }
  ...
  for (int i = 1; i < outsize; i++) {
    ...
    output_spl.push_back(
        output_spl[i - 1] + static_cast<uint64_t>(output_split_sizes[i - 1]));
  }
  for (int i = 1; i < inputsize; i++) {
    input_counts.push_back(static_cast<uint64_t>(input_split_sizes[i]));
    input_spl.push_back(
        input_spl[i - 1] + static_cast<uint64_t>(input_split_sizes[i - 1]));
  }
  ...
  return collective(
      ...
      [&](std::vector<c10_npu::NPUStream>& hcclStreams,
        ...
        if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
          ...
          c10_npu::NPUCachingAllocator::recordStream(
              output_tensors_[0].storage().data_ptr(), hcclStreams[0]);
          if (c10_npu::option::OptionsManager::GetMultiStreamMemoryReuse() ==
              c10_npu::option::ERASE_RECORD_STREAM) {
            work->recorded_outputs_.push_back(std::make_pair(
                output_tensors_[0].storage().getWeakStorageImpl(),
                hcclStreams[0]));
          }
        }
        ...
        for (int i = 0; i < output_results.size(); i++) {
          output_tensors[i].copy_(
              at::reshape(output_results[i], output_tensors[i].sizes()), true);
        }
      },
      c10d::OpType::ALLTOALL);
}
