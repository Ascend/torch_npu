...
NpuP2pCtrl::NpuP2pCtrl() {
  ...
  p2p_access_enabled_cache_.resize(
      num_devices_ * num_devices_, P2pStatus::UNKONWN);
...
bool NpuP2pCtrl::get_p2p_access(
  ...
  if (device_enabled_count_[source_dev] >= C10_P2P_ACCESS_MAX_NPUS ||
    ...
    for (auto i = 0; i < num_devices_; i++) {
      if (p2p_access_enabled_cache_[source_dev * num_devices_ + i] ==
    ...
    ASCEND_LOGW(
        ...
        "Currently NPU device %d has already enable with 8 device, they are %s",
        source_dev,
        warning_str.c_str());
    return static_cast<bool>(cache_s2d);
  }
