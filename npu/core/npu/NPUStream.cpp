...
c10::StreamId makeStreamId(StreamIdType st, size_t si) {
  return static_cast<c10::StreamId>(
      (static_cast<size_t>(st) << kStreamsPerPoolBits) | si);
}
...
static thread_local std::unique_ptr<LeakyStreamInternals*[]> current_streams =
    nullptr;
