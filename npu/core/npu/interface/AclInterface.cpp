...
namespace acl {
#undef LOAD_FUNCTION
#define LOAD_FUNCTION(funcName) REGISTER_FUNCTION(libascendcl, funcName)
#undef GET_FUNC
...
aclError AclrtCreateStreamWithConfig(
  ...
  if (func == nullptr) {
    func =
        (aclrtCreateStreamWithConfigFunc)GET_FUNC(aclrtCreateStreamWithConfig);
  }
  ...
  if (ret == ACL_SUCCESS && stream != nullptr) {
    ...
    if (!c10_npu::IsSupportInfNan()) {
      TORCH_CHECK(
          AclrtSetStreamOverflowSwitch(*stream, 1) == ACL_SUCCESS,
          "SET StreamOverflowSwitch Failed.",
...
aclError AclrtSetStreamFailureMode(aclrtStream stream, uint64_t mode) {
  ...
  static aclrtSetStreamFailureModeFunc func =
      (aclrtSetStreamFailureModeFunc)GET_FUNC(aclrtSetStreamFailureMode);
  if (func == nullptr) {
    return ACL_SUCCESS;
  }
...
bool IsExistCreateEventExWithFlag() {
  ...
  static AclrtCreateEventWithFlagFunc func =
      (AclrtCreateEventWithFlagFunc)GET_FUNC(aclrtCreateEventExWithFlag);
  return func != nullptr;
}
...
aclError AclrtCreateEventWithFlag(aclrtEvent* event, uint32_t flag) {
  ...
  //   3. Only support query event record status, aclrtQueryEvent and
  ...
  if (func == nullptr) {
    TORCH_NPU_WARN_ONCE(
        func, "Failed to find function ", "aclrtCreateEventExWithFlag");
...
aclError AclrtSynchronizeStreamWithTimeout(aclrtStream stream) {
  ...
  static AclrtSynchronizeStreamWithTimeout func =
      (AclrtSynchronizeStreamWithTimeout)GET_FUNC(
  ...
  if (func != nullptr) {
    ...
    TORCH_CHECK(
        func_backup,
        "Failed to find function",
        "aclrtSynchronizeStreamWithTimeout and aclrtSynchronizeStream",
        PROF_ERROR(ErrCode::NOT_FOUND));
...
aclError AclrtDestroyStreamForce(aclrtStream stream) {
  ...
  static AclrtDestroyStreamForce func =
      (AclrtDestroyStreamForce)GET_FUNC(aclrtDestroyStreamForce);
  if (func != nullptr) {
    return func(stream);
  }
  TORCH_NPU_WARN_ONCE(
...
aclError AclrtGetDeviceUtilizationRate(
  ...
  if (func == nullptr) {
    func =
        (AclrtGetDeviceUtilizationRate)GET_FUNC(aclrtGetDeviceUtilizationRate);
  }
  TORCH_CHECK(
      func,
      "Failed to find function ",
...
aclError AclrtMallocAlign32(
  ...
  static AclrtMallocAlign32 func =
      (AclrtMallocAlign32)GET_FUNC(aclrtMallocAlign32);
  if (func != nullptr) {
...
bool IsExistGetCannAttribute() {
  ...
  static AclGetCannAttribute func =
      (AclGetCannAttribute)GET_FUNC(aclGetCannAttribute);
  return func != nullptr;
}
