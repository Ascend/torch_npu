...
#undef LOAD_FUNCTION
#define LOAD_FUNCTION(funcName) REGISTER_FUNCTION(libmsprofiler, funcName)
#undef GET_FUNC
#define GET_FUNC(funcName) GET_FUNCTION(libmsprofiler, funcName)
...
aclError AclprofGetSupportedFeatures(
  ...
  if (func == nullptr) {
    func =
        (AclprofGetSupportedFeaturesFunc)GET_FUNC(aclprofGetSupportedFeatures);
    if (func == nullptr) {
      return ACL_ERROR_PROF_MODULES_UNSUPPORTED;
    }
  }
