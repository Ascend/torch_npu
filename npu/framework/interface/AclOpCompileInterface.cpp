...
namespace native {
#undef LOAD_FUNCTION
#define LOAD_FUNCTION(funcName) REGISTER_FUNCTION(libacl_op_compiler, funcName)
#undef GET_FUNC
#define GET_FUNC(funcName) GET_FUNCTION(libacl_op_compiler, funcName)
...
aclError AclSetCompileopt(aclCompileOpt opt, const char* value) {
  ...
  typedef aclError (*aclSetCompileoptFunc)(
      aclCompileOpt opt, const char* value);
...
aclError AclGetCompileopt(aclCompileOpt opt, char* value, size_t length) {
  typedef aclError (*aclGetCompileoptFunc)(
      aclCompileOpt opt, char* value, size_t length);
...
aclError AclGenGraphAndDumpForOp(
  ...
  typedef aclError (*AclGenGraphAndDumpForOpFunc)(
      const char*,
      int,
...
aclError AclopCompileAndExecuteV2(
    ...
    const char* opPath,
    aclrtStream stream) {
  typedef aclError (*AclopCompileAndExecuteV2Func)(
      const char*,
      int,
      aclTensorDesc*[],
      aclDataBuffer*[],
...
aclError AclrtCtxSetSysParamOpt(aclSysParamOpt opt, int64_t value) {
  typedef aclError (*AclrtCtxSetSysParamOptFunc)(
      aclSysParamOpt opt, int64_t value);
