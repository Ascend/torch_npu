...
class InferFormat {
 public:
  ...
  static std::tuple<aclFormat, aclFormat> GuessFormatUnit(
  ...
  static aclFormat GuessStorageFormat(
      const c10::IntArrayRef& size,
      aclFormat format);
  // Features: guess the format of tensor after it called format_contiguous().
  // According to the law of continuity, the output format is same as input
  // format, this function is called to guess the input format, so it also the
  // output format. NOTE: The caller should make sure that the tensor is
  // non-contigous
  ...
  static FormatShape GuessStorageSizeWhenConvertFormat(
  ...
  // as_strided and view
  static bool IsDefiniteTensorWhenMetaDataChanges(
      const at::Tensor& tensor,
      const c10::IntArrayRef& size);
}; // class InferFormat
